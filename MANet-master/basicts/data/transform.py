...
def standard_transform(data_x: np.array, data_y: np.array, norm_each_channel: int = False) -> np.array:
    ...
    data_train = np.concatenate([data_x, data_y], axis=1)
    ...
    # mode_func = mode + "func"
    # mode_args = mode + "args"
    # scaler[mode_func] = re_standard_transform.__name__
    # scaler[mode_args] = {"mean": mean, "std": std}
    func = re_standard_transform.__name__
    ...
    data_norm_x = normalize(data_x)

    data_norm_y = normalize(data_y)


    return data_norm_x, data_norm_y, func, args
