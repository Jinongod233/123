...
class BaseTimeSeriesForecastingRunner(BaseRunner):
    ...
    def __init__(self, cfg: dict):
        ...
        self.hours = cfg["DATASET_HOUR"]
        self.day = cfg["DATASET_DAY"]
        self.week = cfg["DATASET_WEEK"]

        # read scaler for re-normalization
        self.scaler = load_pkl("{0}/{1}.npz/scaler.pkl".format(cfg["TRAIN"]["DATA"]["DIR"], self.dataset_name))
        # define loss
    ...
    def build_train_dataset(self, cfg: dict):
        ...
        data_file_path = "{0}/{1}_r{2}_d{3}_w{4}.npz".format(cfg["TRAIN"]["DATA"]["DIR"], cfg["DATASET_NAME"], self.hours, self.day, self.week)
        index_file_path = "{0}/{1}_r{2}_d{3}_w{4}_index.npz".format(cfg["TRAIN"]["DATA"]["DIR"], cfg["DATASET_NAME"], self.hours, self.day, self.week)
        # index_file_path = "{0}/index_in{1}_out{2}.pkl".format(cfg["TRAIN"]["DATA"]["DIR"], cfg["DATASET_INPUT_LEN"], cfg["DATASET_OUTPUT_LEN"])
        ...
        dataset_target, dataset_x, dataset_index = dataset['train']
        dataset_target_tensor = torch.tensor(dataset_target).type(torch.FloatTensor)
        dataset_x_tensor = torch.tensor(dataset_x).type(torch.FloatTensor)
        dataset_index_tensor = torch.tensor(dataset_index).type(torch.FloatTensor)
        dataset_x_tensor = torch.cat((dataset_x_tensor, dataset_index_tensor), dim=-1)
        print("train len: {0}".format(dataset_x_tensor.shape[0]))

        batch_size = cfg["TRAIN"]["DATA"]["BATCH_SIZE"]
        self.iter_per_epoch = math.ceil(dataset_x_tensor.shape[0] / batch_size)

        train_dataset = torch.utils.data.TensorDataset(dataset_target_tensor, dataset_x_tensor)
    ...
    def build_val_dataset(cfg: dict):
        ...
        data_file_path = "{0}/{1}_r{2}_d{3}_w{4}.npz".format(cfg["VAL"]["DATA"]["DIR"], cfg["DATASET_NAME"], cfg["DATASET_HOUR"], cfg["DATASET_DAY"], cfg["DATASET_WEEK"])
        index_file_path = "{0}/{1}_r{2}_d{3}_w{4}_index.npz".format(cfg["VAL"]["DATA"]["DIR"], cfg["DATASET_NAME"], cfg["DATASET_HOUR"], cfg["DATASET_DAY"], cfg["DATASET_WEEK"])
        # build dataset args
        dataset_args = cfg.get("DATASET_ARGS", {})
        # three necessary arguments, data file path, corresponding index file path, and mode (train, valid, or test)
        dataset_args["data_file_path"] = data_file_path
        ...
        dataset = cfg["DATASET_CLS"](**dataset_args)
        dataset_target, dataset_x, dataset_index = dataset['valid']
        dataset_target_tensor = torch.tensor(dataset_target).type(torch.FloatTensor)
        dataset_x_tensor = torch.tensor(dataset_x).type(torch.FloatTensor)
        dataset_index_tensor = torch.tensor(dataset_index).type(torch.FloatTensor)
        dataset_x_tensor = torch.cat((dataset_x_tensor, dataset_index_tensor), dim=-1)
        print("valid len: {0}".format(dataset_x_tensor.shape[0]))

        valid_dataset = torch.utils.data.TensorDataset(dataset_target_tensor, dataset_x_tensor)
    ...
    def build_test_dataset(cfg: dict):
        ...
        data_file_path = "{0}/{1}_r{2}_d{3}_w{4}.npz".format(cfg["TEST"]["DATA"]["DIR"], cfg["DATASET_NAME"], cfg["DATASET_HOUR"], cfg["DATASET_DAY"], cfg["DATASET_WEEK"])
        index_file_path = "{0}/{1}_r{2}_d{3}_w{4}_index.npz".format(cfg["TEST"]["DATA"]["DIR"], cfg["DATASET_NAME"], cfg["DATASET_HOUR"], cfg["DATASET_DAY"], cfg["DATASET_WEEK"])
        ...
        dataset = cfg["DATASET_CLS"](**dataset_args)
        dataset_target, dataset_x, dataset_index = dataset['test']
        dataset_target_tensor = torch.tensor(dataset_target).type(torch.FloatTensor)
        dataset_x_tensor = torch.tensor(dataset_x).type(torch.FloatTensor)
        dataset_index_tensor = torch.tensor(dataset_index).type(torch.FloatTensor)
        dataset_x_tensor = torch.cat((dataset_x_tensor, dataset_index_tensor), dim=-1)
        print("test len: {0}".format(dataset_x_tensor.shape[0]))

        test_dataset = torch.utils.data.TensorDataset(dataset_target_tensor, dataset_x_tensor)
    ...
    def train_iters(self, epoch: int, iter_index: int, data: Union[torch.Tensor, Tuple]) -> torch.Tensor:
        ...
        # re-scale data
        prediction_rescaled = SCALER_REGISTRY.get(self.scaler["train_func"])(forward_return[0], **self.scaler["train_args"])
        real_value_rescaled = SCALER_REGISTRY.get(self.scaler["train_func"])(forward_return[1], **self.scaler["train_args"])
        # loss
        if self.cl_param:
    ...
    def val_iters(self, iter_index: int, data: Union[torch.Tensor, Tuple]):
        ...
        # re-scale data
        prediction_rescaled = SCALER_REGISTRY.get(self.scaler["val_func"])(forward_return[0], **self.scaler["val_args"])
        real_value_rescaled = SCALER_REGISTRY.get(self.scaler["val_func"])(forward_return[1], **self.scaler["val_args"])
    ...
    def test(self):
        ...
        # re-scale data
        prediction = SCALER_REGISTRY.get(self.scaler["test_func"])(
            prediction, **self.scaler["test_args"])
        real_value = SCALER_REGISTRY.get(self.scaler["test_func"])(
            real_value, **self.scaler["test_args"])
